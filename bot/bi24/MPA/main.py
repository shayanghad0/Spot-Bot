...
def export_balance_png(filename):
    iran_date = get_iran_date()
    trade_file = f"trade-{iran_date}.json"
    if not os.path.exists(trade_file):
        print("  No trade file found for PNG export.")
        return None
    ...
    start_balance = _bot_state.get("start_balance", 0)
    quote = _bot_state.get("quote", "USDT")

    # Build balance progression
    balance = start_balance
    balance_points = [balance]
    labels = ["Start"]

    for t in trades:
        spent = t.get("amount_currency", 0)
        balance -= spent
        balance_points.append(balance)
        labels.append(f"Buy #{t.get('ID','')}")

        sell_p = t.get("sell_price")
        if sell_p:
            coin = t.get("coinReceived", 0)
            sell_fee = t.get("sell_fee", 0)
            received = coin * sell_p - sell_fee
            balance += received
            balance_points.append(balance)
            labels.append(f"Sell #{t.get('ID','')}")

    end_balance = balance
    _bot_state["end_balance"] = end_balance

    # Format currency without dots for IRT, with dots for USDT
    def fmt(val):
        if quote == "IRT":
            return f"{int(val):,}"
        return f"{val:,.2f}"
    ...
    x = range(len(balance_points))
    colors = ['#42a5f5' if b >= start_balance else '#ff5252' for b in balance_points]

    ax.plot(x, balance_points, color='#00e676', linewidth=2, marker='o', markersize=6,
            markerfacecolor='#00e676', markeredgecolor='white', markeredgewidth=1.5, zorder=5)
    ax.fill_between(x, balance_points, alpha=0.15, color='#00e676')

    # Start line
    ax.axhline(y=start_balance, color='#ffc107', linestyle='--', linewidth=1, alpha=0.7,
               label=f'Start: {fmt(start_balance)} {quote}')

    # End point
    ax.plot(len(balance_points)-1, end_balance, marker='*', markersize=18,
            color='#00e676', markeredgecolor='white', markeredgewidth=1.5, zorder=6)
    ax.annotate(f'End: {fmt(end_balance)} {quote}',
                xy=(len(balance_points)-1, end_balance),
    ...
    ax.text(0.02, 0.95, f'P&L: {fmt(pnl)} {quote} ({pnl_pct:+.2f}%)',
            transform=ax.transAxes, color=pnl_color, fontsize=13, fontweight='bold',
            verticalalignment='top',
            bbox=dict(boxstyle='round,pad=0.5', facecolor='#1a1a2e', edgecolor=pnl_color, alpha=0.9))

    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha='right', color='#888', fontsize=9)
    ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda v, p: fmt(v)))
    ax.set_ylabel(f'Balance ({quote})', color='#888', fontsize=12)
    ax.set_title(f'{_bot_state.get("symbol","")}/{quote} Balance Progression',
                 color='#e0e0e0', fontsize=16, fontweight='bold', pad=15)
    ...
    ax.legend(loc='upper right', fontsize=10, facecolor='#1a1a2e', edgecolor='#333',
              labelcolor='#e0e0e0')
